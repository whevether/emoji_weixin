...
class GiphySearchTab extends StatefulWidget {
  const GiphySearchTab({
    super.key,
  ...
  final Future<Sticker> Function(GiphyGifItem item) onAddToFavorite;
  final void Function(Sticker sticker) onSelected;

  @override
  State<GiphySearchTab> createState() => _GiphySearchTabState();
}

class _GiphySearchTabState extends State<GiphySearchTab> {
  final _controller = TextEditingController();
  Timer? _debounce;
  bool _stickersMode = true;
  bool _loading = false;
  String? _error;
  List<GiphyGifItem> _items = const [];
  ...
  Future<void> _loadTrending() async {
    ...
    try {
      final items = await widget.client.trendingStickers();
      if (!mounted) return;
      setState(() {
        _items = items;
        _loading = false;
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return InkWell(
                onTap: () async {
                  final sticker = await widget.onAddToFavorite(item);
                  widget.onSelected(sticker);
                },
                onLongPress: () {
                  // Preview as ephemeral network sticker without saving.
                  widget.onSelected(
                    Sticker(
                      id: item.id,
                      packId: 'giphy',
